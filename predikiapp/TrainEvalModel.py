...
def TrainModelForMultiLabel(algorithm, base, training_df, num_labels, args, weight=None, use_cuda=True):
...
def EvalFromModel(model, evaluation_df):
    import sklearn
    # Evaluate the model
    result, model_outputs, wrong_predictions = model.eval_model(evaluation_df, acc=sklearn.metrics.accuracy_score,
                                                                matr=sklearn.metrics.confusion_matrix)
